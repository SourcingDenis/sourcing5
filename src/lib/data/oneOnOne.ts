...
export async function generateAgenda(
  ...
  if (reportFunnelSummary) {
    ...
    if (totalOutreach > 0) winsBullets.push(`${totalOutreach} outreach messages sent this week`);
    if (avgReplyRate >= 0.2) {
      winsBullets.push(`Strong reply rate: ${(avgReplyRate * 100).toFixed(0)}% (above 20% threshold)`);
    }
    if (totalScreens > 0) winsBullets.push(`${totalScreens} screen(s) booked across all reqs`);
  }
  if (winsBullets.length === 0) winsBullets.push('No funnel wins captured this week — discuss qualitative progress');

  // 2. CAPACITY REVIEW
  const capacityBullets: string[] = [
    `Load ratio: ${(loadRatio * 100).toFixed(0)}% (${assignedHours}h allocated / ${totalCapacity}h capacity)`,
  ];
  if (loadRatio > 1.0) capacityBullets.push(`Over capacity — discuss re-prioritization`);
  else if (loadRatio < 0.5 && totalCapacity > 0) capacityBullets.push(`Under-utilized — explore stretch goals`);
  ...
  for (const alert of funnelAlerts) {
    const prefix = alert.severity === 'red' ? '[RED]' : alert.severity === 'warning' ? '[WARNING]' : '[FLAG]';
    funnelBullets.push(`${prefix} ${alert.reqTitle}: ${alert.message}`);
  }
  for (const stalled of stalledReqs) {
    funnelBullets.push(`[STALLED] ${stalled.reqTitle} — no outreach in ${stalled.daysSince} days`);
  }
  if (funnelBullets.length === 0) funnelBullets.push('All funnels healthy this week');
  ...
  if (blockerBullets.length === 0) blockerBullets.push('No open action items');
  ...
  const sections: AgendaSection[] = [
    ...
    { section: 'development', label: 'Development Focus',           bullets: ['Career goals check-in', 'Skill development'] },
  ];
...
export async function getManagerDashboardStats(
  ...
  const reportIds = allUsers.filter((u) => u.managerId === managerId).map((u) => u.id);
  ...
  const calcRate = (ones: OneOnOne[]) => {
    if (ones.length === 0) return 0;
    const completedOnTime = ones.filter(o => o.completedAt && new Date(o.completedAt) <= new Date(new Date(o.scheduledAt).getTime() + 24 * 60 * 60 * 1000));
    return completedOnTime.length / ones.length;
  };
